...
While the coefficients in our regression tell us *what* the estimated relationship is, our **standard errors** tell us *how confident* we are in that relationship. Getting standard errors right is critical: if they are wrong, then t-statistics, p-values, and confidence intervals are all wrong — even if the coefficient itself is unbiased. This lecture builds significantly on [Michler and Josephson (2022)](https://doi.org/10.4337/9781800372054.00019), a comprehensive review of recent developments in inference for applied economics.
...
* export comparison table
	esttab          hc1 hc2 hc3 using "$answ/13-se-hcse.tex", replace ///
	                    b(3) se(3) ///
	                    star(* 0.10 ** 0.05 *** 0.01) ///
	                    mtitles("HC1 (robust)" "HC2" "HC3") ///
	                    ...
	                    prehead("\begin{tabular}{l*{3}{c}} " ///
	                        ...
	                        "& \multicolumn{3}{c}{Heteroskedasticity-Consistent SEs} " ///
	                        "\\ \midrule") ///
	                    postfoot("\hline \hline \\[-1.8ex] " ///
	                        "\multicolumn{4}{p{0.85\linewidth}}{\small " ///
	                        "\noindent \textit{Note}: Dependent variable " ///
	                        "is life expectancy. All models use OLS. " ///
...
Let's put all our SE methods side by side:

```stata
* compile a final comparison table
	esttab          hc1 hc2 hc3 boot clust ///
	                    using "$answ/13-se-comparison.tex", replace ///
	                    b(3) se(3) ///
	                    star(* 0.10 ** 0.05 *** 0.01) ///
	                    mtitles("HC1" "HC2" "HC3" "Bootstrap" "Clustered") ///
	                    stats(N r2, labels("Observations" "R-squared") ///
	                          fmt(0 3)) ///
	                    noobs booktabs nonum collabels(none) ///
	                    nobaselevels nogaps fragment label ///
	                    prehead("\begin{tabular}{l*{5}{c}} " ///
	                        "\\[-1.8ex]\hline \hline \\[-1.8ex] " ///
	                        "& \multicolumn{5}{c}{Standard Error Method} " ///
	                        "\\ \midrule") ///
	                    postfoot("\hline \hline \\[-1.8ex] " ///
	                        "\multicolumn{6}{p{\linewidth}}{\small " ///
	                        "\noindent \textit{Note}: Dependent variable " ///
	                        "is life expectancy. Coefficients are " ///
	                        "identical across all columns. Only the " ///
	                        "standard errors (in parentheses) differ. " ///
	                        "Bootstrap uses 1,000 repetitions. " ///
	                        "Clustering is at the region level (3 clusters). " ///
	                        "* p$<$0.10, ** p$<$0.05, *** p$<$0.01.} " ///
	                        "\end{tabular}")
```
