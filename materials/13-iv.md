...
- The mechanics and intuition of Two-Stage Least Squares (2SLS)  
- Performing 2SLS manually to understand the method  
- Using Stata's `ivregress` command  
- Diagnostic tests for instrument validity  
- Panel IV with `xtivreg`  
- Exporting IV results to LaTeX tables and coefficient plots
...
```text
lwage = β₀ + β₁ · educ + β₂ · exper + β₃ · expersq + ε
```
...
**A critical caveat**: while the *coefficients* from manual 2SLS are correct, the *standard errors* are wrong. Stata doesn't know that `educ_hat` is an estimated variable rather than a real observed one, so it underestimates the uncertainty. We'll fix this with `ivregress` next.

Let's export our comparison so far:

```stata
* compare OLS and manual 2SLS in a table
	esttab          ols manual_2sls using "$answ/13-iv-manual.tex", replace ///
	                    b(3) se(3) ///
	                    keep(educ educ_hat exper expersq) ///
	                    star(* 0.10 ** 0.05 *** 0.01) ///
	                    ...
	                    postfoot("\hline \hline \\[-1.8ex] " ///
	                        ...
	                        "\noindent \textit{Note}: Manual 2SLS " ///
	                        "standard errors are incorrect — they " ///
	                        "do not account for the generated regressor. " ///
...
Doing 2SLS by hand is intuitively useful, but we should always use Stata's built-in `ivregress` command for actual analysis. It runs both stages simultaneously and correctly adjusts the standard errors. The syntax requires us to specify the endogenous variable and the instruments in parentheses:
...
While `ivregress` is the built-in Stata command for IV, the user-written `ivreg2` package (Baum, Schaffer, and Stillman) is the preferred command among applied researchers. `ivreg2` has the same basic syntax as `ivregress` but provides several important advantages:
...
- **More VCE options**: It natively supports clustered, two-way clustered, HAC (Newey-West), and other standard error options.  
...
Install `ivreg2` by adding it to the package loop in your `project.do` file (you'll also need the `ranktest` package). The syntax is nearly identical to `ivregress`:
...
Notice: the coefficient on `educ` is identical to the manual 2SLS, but the standard errors will be different (and correct). Let's see all three side by side:

```stata
* three-column comparison: OLS, Manual 2SLS, ivregress
	esttab          ols manual_2sls iv_auto ///
	                    using "$answ/13-iv-comparison.tex", replace ///
	                    b(3) se(3) ///
	                    keep(educ educ_hat exper expersq) ///
	                    star(* 0.10 ** 0.05 *** 0.01) ///
	                    mtitles("OLS" "Manual 2SLS" "IV (ivregress)") ///
	                    indicate("Controls = exper expersq") ///
	                    stats(N r2, labels("Observations" "R-squared") ///
	                          fmt(0 3)) ///
	                    noobs booktabs nonum collabels(none) ///
	                    nobaselevels nogaps fragment label ///
	                    ...
	                    postfoot("\hline \hline \\[-1.8ex] " ///
	                        ...
	                        "is log wage. Column 1 is OLS. Column 2 is " ///
	                        "manual 2SLS (standard errors are incorrect). " ///
	                        "Column 3 is \texttt{ivregress} with robust " ///
	                        "standard errors. " ///
	                        "* p$<$0.10, ** p$<$0.05, *** p$<$0.01.} " ///
	                        "\end{tabular}")
```
...
```stata
* coefficient plot: OLS vs IV
	coefplot        (ols, label("OLS")) ///
	                (iv_auto, label("IV (2SLS)")), ///
	                    keep(educ) xline(0) ///
	                    title("Returns to Education: OLS vs IV") ///
	                    xtitle("Coefficient on Education") ///
...
Just because we declare a variable an instrument doesn't magically make it a valid one. Stata provides a suite of post-estimation diagnostic tests that we should always run.

**1. Relevance (Weak Instruments)**

If our instruments don't strongly predict the treatment variable, they are "weak" and will amplify bias rather than fix it. We test this by running `estat firststage` after `ivregress`. A common rule of thumb is to look for an F-statistic greater than 10, though modern econometrics often demands higher thresholds (Stock and Yogo, 2005).
...
When we have more instruments than endogenous variables (our case: 2 instruments, 1 endogenous variable), we are *overidentified*. This allows us to test whether our instruments are valid (i.e., uncorrelated with the error term) using `estat overid`. If the p-value is $> 0.05$, we fail to reject the null that our instruments are valid.
...
Interpreting the results: the first-stage F-statistic should exceed 10 (relevance). The endogeneity test tells us whether IV is necessary. The overidentification test tells us whether our "extra" instrument is consistent with the exclusion restriction. Together, these three tests form your complete diagnostic checklist.

> Do [Exercise 3 - Panel IV with `xtivreg2`]({{ site.baseurl }}/exercises/13-iv-panel/)

### Panel Data and `xtivreg`

While `ivregress` handles cross-sectional data, what if we have panel data and want to absorb unobserved time-invariant heterogeneity using fixed effects, while *also* instrumenting a variable?

Stata handles this through the `xtivreg` command. It combines the mechanics of `xtreg, fe` with `ivregress 2sls`. The syntax is:

```stata
* panel iv syntax (conceptual example)
* assumes a panel dataset with xtset id year
...
Just as `ivreg2` improves on `ivregress`, the `xtivreg2` package (also by Baum, Schaffer, and Stillman) extends `xtivreg` with the same suite of enhanced diagnostics:
...
```stata
* panel iv with xtivreg2 (conceptual example)
	xtivreg2        y exogenous_controls (endogenous_var = instruments) ///
	                    i.year, fe cluster(id) first
```
...
- Always use `ivregress` (or `ivreg2`) rather than manual 2SLS — the standard errors from manual 2SLS are incorrect  
- `ivreg2` is preferred over `ivregress` because it automatically reports weak instrument tests, overidentification statistics, and supports additional estimators (LIML, GMM)  
