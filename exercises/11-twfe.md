...
- Now estimate the exact same model using the `areg` command. Use `i.year` to include time fixed effects, and specify `absorb(panelid)` to absorb the individual fixed effects. Remember to cluster standard errors.
