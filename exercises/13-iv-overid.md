...
- Ensure you have loaded `Mroz.dta`. Run the standard, non-robust `ivregress` (the overidentification test requires homoskedastic standard errors):
...
- Run the command `estat overid`. 
- This runs a Sargan test to check if your instruments are uncorrelated with the error term. If the $p$-value is $>0.05$, we fail to reject the null that our instruments are valid. 

1. Based on the output, did our instruments pass the exogeneity test?
