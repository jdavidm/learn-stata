...
- Load `lifeexp.dta`.
- Run Stata's default robust regression (HC1):
...
- Run the leverage-adjusted robust regression (HC2) to correct for the outliers:
...
1. Compare the standard error for `gnppc` under HC1 vs HC2. Does adjusting for leverage increase or decrease our confidence (standard error) in the estimate?
