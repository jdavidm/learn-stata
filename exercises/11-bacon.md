...
2\. Now run `csdid yield, ivar(qnno) time(year) gvar(first_icp) tr(icp)`. Calculate the aggregated overall ATT by running `estat event` and use `eststo` to save the result. Then use `esttab` to create a table of the results from this model and the TWFE model you estimated in the last exercise.
