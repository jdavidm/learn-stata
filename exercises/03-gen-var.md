...
2\. Compute the mean total consumption in dollars (`meancons_USD`) for each consumption quintile using the `by()` option along with `egen`. Label the new variable with "Mean consumption per quntile (USD)". Using `tab`, what is the mean consumption for the highest and lowest qunitle?
