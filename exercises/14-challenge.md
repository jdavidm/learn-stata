...
- Run the following five specifications using either `reghdfe` or `ivreghdfe` as appropriate, all with triangular kernel weights, district-threshold and year FE, and standard errors clustered at the village level:
  1. **First stage**: regress `receivedroad` on `t`, `left`, `right`, and `$blcontrols`. Store as `fs`.
  2. **Reduced-form fires**: regress `fires10km` on `t`, `left`, `right`, `fires2001_10km`, and `$blcontrols`. Store as `rf_fires`.
  3. **IV fires**: regress `fires10km` on `left`, `right`, `fires2001_10km`, and `$blcontrols`, treating `receivedroad` as endogenous and `t` as the instrument. Store as `iv_fires`.
