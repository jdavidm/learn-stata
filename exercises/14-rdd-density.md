...
- Run the McCrary density test using the `dc_density` command (make sure you've downloaded the `00_setup.do` file from the replication package and copied the `dc_density` program into your `project.do` file first):
  `dc_density v_pop, breakpoint(0) generate(Xj Yj r0 fhat se_fhat) graphname("$answ/14-rdd-density.eps")`
...
- Alternatively, or additionally, to match the formatting of our other graphs, create a standard histogram of the running variable with a vertical line at the cutoff:
...
* custom histogram of running variable
	...
	graph export	"$answ/14-rdd-density-hist.png", replace
```

1\. What is the estimated log-difference in height at the cutoff (the parameter `theta` in the McCrary test output) and its standard error? Is there evidence of manipulation?

2\. Why is manipulation unlikely in this setting? Think about what the running variable is and whether villages could plausibly change it.
