...
2. You can also download data to your machine, placing it in a specific folder, and then use the absolute path to load the data useing `use`. Download the file `tenuredata.dta` from the [course github repo](https://github.com/jdavidm/semester26). Take care to note where you save this file. Then load the data into stata and `describe` the data.
...
4. Stata can also input almost any type of data file and convert it to a `.dta` format. To do this, you use a compound command that starts with `import` and is followed by the format of the file. Type in `help import` to get a list of all the different types of files Stata can import. Download the file `district_size.csv` from the [course github repo](https://github.com/jdavidm/semester26). Again, take care to note where you saved the file. Then use `help` to find the Stata command to import a comma-separated (the `cs` in `.csv`) file. Describe the data.
