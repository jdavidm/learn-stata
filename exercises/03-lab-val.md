...
Defining value labels when there are just a couple options (yes/no, male/female) is fairly straightforward. But defining value labels when there are a lot of options can become very tedious very quickly. Luckily, Stata has a decode/encode command that can sometimes creatly reduce the amount of coding you have to do to define and assign a label.

You will often have a variable which is a string that you would like to convert to a numeric. As an example, you can think of a variable called `state` that takes values of the name of each state in the U.S. In order to use this information in a regression, we need to convert the string to a numeric. (You will see one way to do this in [Exercise 5 - Create Variables](https://jdavidm.github.io/learn-stata/exercises/03-gen-var/). But with 50 state names, it will take a long time to 1) create a new variable that takes a unique numerical value for each state name and 2) define a label set for every value.
