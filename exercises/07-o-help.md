...
In this exercise you will practice using online resources (Statalist, Stack Overflow, etc.) to solve a coding problem and adapt example code safely. For exercise 1-3 you developed and partially implemented a plan to turn the plot-level data set into a household-season level data set. In laying out that plan you most likely proposed using the `collapse` function to aggregate each row (plot) into a new data set where each row represented a household-season.
...
2\. To verify that you adapted the code correctly, run a regression with the household-season mean yield on the left-hand side and household-season mean labor days and nitrogen fertilizer on the right-hand side. What are the coefficients on `mean_labor` and `mean_fert`?
